...
1. `AnimatedBuilder`表示一个动画的构造器，可以通过控制器去控制动画值改变，从而控制动画
...
AnimatedBuilder({
    Key key,
    Listenable animation,      //动画属性值的变化
    this.builder,              //动画控制器，参数包含child
    this.child,                //等同于动画控制器中child的参数
})
```
...
* child：表示由外传递进来的元素，由`Container`包裹
* animation：表示由外传递进来的动画属性值的变化，通过获取其值，填充到`Container`宽和高中
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20190730232440346.gif)
