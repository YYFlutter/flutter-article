...
1. `FadeTransition`表示一个透明度动画，可以通过控制器去控制动画透明度值的改变，从而控制动画的透明度
...
FadeTransition({
    Key key,
    this.opacity,                    //动画透明度值的变化
    this.alwaysIncludeSemantics,     //是否包含子语义而不管不透明度
    Widget child,                    //动画子元素
})
```
...
* child：表示由外传递进来的元素，由`FadeTransition`包裹
* opacity：表示由外传递进来的动画属性值的变化，通过获取其值，填充到`child`的透明度上
* alwaysIncludeSemantics：表示是否包含子语义而不管不透明度
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20190801171433453.gif)
